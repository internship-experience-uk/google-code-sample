...
def test_create_existing_playlist(capfd):
  ...
  out, err = capfd.readouterr()
  assert "Successfully created new playlist: my_playlist" in out
  assert ("Can not create playlist: A playlist with the same name already "
          "exists.") in out
...
def test_add_to_playlist_none_existant(capfd):
  ...
  out, err = capfd.readouterr()
  assert "Can not add video to another_playlist. Playlist does not exist." in out
...
def test_remove_from_playlist_nonexistant_video(capfd):
  ...
  assert "Successfully created new playlist: my_playlist" in out
  assert "Added video to my_playlist: Amazing Cats" in out
  assert "Can not remove video from my_playlist: Video does not exist." in out
...
def test_clear_playlist_nonexistant(capfd):
  ...
  player.clear_playlist("my_playlist")
  out, err = capfd.readouterr()
  assert "Can not clear playlist another_playlist: Playlist does not exist." in out
...
def test_delete_playlist_nonexistant(capfd):
  ...
  player.delete_playlist("my_playlist")
  out, err = capfd.readouterr()
  assert "Can not delete playlist my_playlist: Playlist does not exist." in out
