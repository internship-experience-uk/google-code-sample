...
TEST(Part1, stopVideoTwice)
{
    ...
    EXPECT_THAT(output, HasSubstr("Playing video: Amazing Cats"));
    EXPECT_THAT(output, HasSubstr("Stopping video: Amazing Cats"));
    EXPECT_THAT(output, HasSubstr("Cannot stop video: No video is currently playing"));
}
...
TEST(Part1, playRandomVideo)
{
    ...
    std::string output = testing::internal::GetCapturedStdout();
    EXPECT_THAT(output, ContainsRegex("Playing video: (Amazing Cats|Another Cat Video|Funny Dogs|Life at Google|Video about nothing)"));
}
...
TEST(Part1, showPlaying)
{
    ...
    std::string output = testing::internal::GetCapturedStdout();
    EXPECT_THAT(output, HasSubstr("Playing video: Amazing Cats"));
    EXPECT_THAT(output, HasSubstr("Currently playing: Amazing Cats (amazing_cats_video_id) [#cat #animal]"));
}
...
TEST(Part1, pauseVideoShowVideo)
{
    ...
    EXPECT_THAT(output, HasSubstr("Playing video: Amazing Cats"));
    EXPECT_THAT(output, HasSubstr("Pausing video: Amazing Cats"));
    EXPECT_THAT(output, HasSubstr("Currently playing: Amazing Cats (amazing_cats_video_id) [#cat #animal] - PAUSED"));
}
...
TEST(Part1, pauseVideoNothingPlaying)
{
    ...
    videoPlayer.pauseVideo();
    std::string output = testing::internal::GetCapturedStdout();
    EXPECT_THAT(output, HasSubstr("Cannot pause video: No video is currently playing"));
}
...
TEST(Part1, continueVideoNothingPlaying)
{
    ...
    videoPlayer.continueVideo();
    std::string output = testing::internal::GetCapturedStdout();
    EXPECT_THAT(output, HasSubstr("Cannot continue video: No video is currently playing"));
}